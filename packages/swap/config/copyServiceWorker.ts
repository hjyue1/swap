...
export default async function copyServiceWorker(
  ...
  const [readError, fileContent] = await until(() => fs.promises.readFile(sourceFilePath, 'utf8'));
  ...
  const [writeFileError] = await until(() => fs.promises.writeFile(destFilePath, nextFileContent));
