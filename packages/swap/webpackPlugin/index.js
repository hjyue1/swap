...
/**
 * Mock Service Worker (1.0.11).
 * - Please do NOT modify this file.
 * - Please do NOT serve this file on production.
 */

const INTEGRITY_CHECKSUM = 'c114f472ae1dec55ef7db5cb550f815b'
...
async function handleRequest(event, requestId) {
  ...
  // Ensure SWAP is active and ready to handle the message, otherwise
...
async function getResponse(event, client, requestId) {
  ...
  // means that SWAP hasn't dispatched the "MOCK_ACTIVATE" event yet
  ...
  // Send the request to the client-side SWAP.
  ...
  switch (clientMessage.type) {
    ...
    case 'INTERNAL_ERROR': {
      ...
      console.error(
        `\
[SWAP] Uncaught exception in the request handler for "%s %s":

${parsedBody.location}

This exception has been gracefully handled as a 500 response, however, it's strongly recommended to resolve this error, as it indicates a mistake in your code. If you wish to mock an error response\
`,
        request.method,
        request.url,
      )
  ...
  return event.respondWith(
    handleRequest(event, requestId).catch((error) => {
      if (error.name === 'NetworkError') {
        console.warn(
          '[SWAP] Successfully emulated a network error for the "%s %s" request.',
          request.method,
          request.url,
        )
        return
      ...
      console.error(
        `\
[SWAP] Caught an exception from the "%s %s" request (%s). This is probably not a problem with Mock Service Worker. There is likely an additional logging output above.`,
        request.method,
        request.url,
