  ...
  return function start(options?: StartOptions) {
    ...
    const startWorkerInstance = async () => {
      if (!('serviceWorker' in navigator)) {
        throw new Error(
          `[SWAP] Failed to register a Service Worker: this browser does not support Service Workers (see https://caniuse.com/serviceworkers), or your application is running on an insecure host (consider using HTTPS for custom hostnames).`,
        )
      }
      ...
      if (!worker) {
        const missingWorkerMessage = options?.findWorker
          ? `[SWAP] Failed to locate the Service Worker registration using a custom "findWorker" predicate.
...
Please consider using a custom "serviceWorker.url" option to point to the actual worker script location, or a custom "findWorker" option to resolve the Service Worker registration manually.`
      ...
      if (integrityError) {
        console.error(`\
[SWAP] Detected outdated Service Worker: ${integrityError.message}
