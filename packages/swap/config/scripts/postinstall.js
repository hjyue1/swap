...
function postinstall() {
  // 1. Check if "package.json" has "swap.workerDirectory" property set.
  const packageJson = JSON.parse(
    fs.readFileSync(path.resolve(parentPackageCwd, 'package.json'), 'utf8'),
  )

  if (!packageJson.swap || !packageJson.swap.workerDirectory) {
    return
  }

  // 2. Check if the worker directory is an existing path.
  const { workerDirectory } = packageJson.swap
  ...
  if (!fs.existsSync(absoluteWorkerDirectory)) {
    return console.error(
      `[SWAP] Failed to automatically update the worker script at "%s": given path does not exist.`,
      workerDirectory,
    )
  }
  ...
  try {
    ...
    console.error(
      `[SWAP] Failed to automatically update the worker script:\n${error}`,
    )
  }
}
