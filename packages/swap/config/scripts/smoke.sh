...
TARBALL_FILENAME="swap-$COMMIT_HASH.tgz"
LOCAL_PACKAGE_PATH="$PWD/$TARBALL_FILENAME"

echo "Mock Service Worker state at commit $COMMIT_HASH"

# Pack the local build of the `swap` package
echo "Packing 'swap' into $LOCAL_PACKAGE_PATH"
...
git clone https://github.com/swapjs/examples.git ~/examples
...
# Use the local build of the `swap` package.
...
echo "Installing the custom build of the 'swap' package..."
yarn use-packed-swap "$TARBALL_FILENAME"
...
matches=$(grep "\"swap\": \"file:" examples/**/package.json || echo "")
read -ar deps_matches <<< "$matches"

if [ ${#examples_list[@]} -gt ${#deps_matches[@]} ]
then
  echo "Found some examples not using the custom swap build!"
...
echo "All examples are using the custom build of swap!"
