...
`)
  ...
  const { swap: swapConfig } = packageJson

  if (swapConfig && swapConfig.workerDirectory === relativePublicDir) {
    return
  }

  // When called `swap init --no-save` do not show the save suggestions.
  if (save === false) {
    return
  }
...
we recommend saving the path to the worker directory in your package.json.
  ...
  if (swapConfig.workerDirectory !== relativePublicDir) {
    console.log(
      `\
${chalk.yellowBright(
  'WARN',
)} The "swap.workerDirectory" in your package.json ("%s")
is different from the worker directory used right now ("%s").
    `,
      swapConfig.workerDirectory,
      relativePublicDir,
    )
    ...
    return promptWorkerDirectoryUpdate(
      `Do you wish to use "${relativePublicDir}" instead of "${swapConfig.workerDirectory}" as the worker directory?`,
      packageJsonPath,
      relativePublicDir,
    )
  }
...
function saveWorkerDirectory(packageJsonPath, publicDir) {
  ...
  console.log(
    chalk.gray('Writing "swap.workerDirectory" to "%s"...'),
    packageJsonPath,
  )
